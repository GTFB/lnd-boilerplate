...
export const CookieConsent: React.FC<CookieConsentProps> = ({
  ...
  // Fallback state for when context is unavailable
  const [showBanner, setShowBanner] = React.useState(false)
  const [contextAvailable, setContextAvailable] = React.useState(true)

  // Always call useCookie hook at the top level
  let cookieContext = null
  try {
    cookieContext = useCookie()
  } catch (error) {
  ...
  // If context is not available, use fallback logic
  if (!cookieContext) {
  ...
  const { showBanner: contextShowBanner, acceptAll, decline, savePreferences } = cookieContext

  if (!contextShowBanner) return null
